...
#cap = cv2.VideoCapture(r"C:\Users\tushar.m\Desktop\122.mp4")
...
#check for HandOn detection
    frame,Threshold_hands = handsOnSteeringDetection(cap,detector,ctime,ptime)
#display the text on picture
    cv2.imshow('frame1', frame)
    #check head and shoulder movement
   # shoulderheadFrame,val = shoulderHeadDetection(frame,mp_pose,pose,mp_drawing)
   ...
   # cv2.imshow('frame2', shoulderheadFrame)
    ...
    if validate_tuple_stays_empty(keypoints_tuple,counterStart,driverFocusedEyeThreshold):
        ...
        #if counterStart >= driverFocusedEyeThreshold and val == 1:
         #   print("Driver shoulder not foucused")
        if counterStart >= driverFocusedEyeThreshold and Threshold_hands == 1:
            print("Driver shoulder not foucused")
           # message = "DM.Brake 1"
